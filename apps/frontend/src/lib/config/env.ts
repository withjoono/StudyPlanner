...
interface EnvConfig {
  ...
  // 백엔드 API URL
  apiUrlPlanner: string; // gb-planner 백엔드
  apiUrlMain: string; // GB-Back-Nest (인증, 결제, 회원)
...
export const env: EnvConfig = {
  ...
  apiUrlPlanner: isDev
    ? '/api'
    : getEnvVar('VITE_API_URL_PLANNER', 'http://localhost:4000'),
  apiUrlMain: isDev
    ? '/api-main'
    : getEnvVar('VITE_API_URL_MAIN', 'http://localhost:4001'),
