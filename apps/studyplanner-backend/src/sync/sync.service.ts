...
export class SyncService {
  ...
  async collectDailyScoreData(studentId: number, date?: string) {
    ...
    // (quiz 모듈 제거됨 — quizResult 쿼리 삭제)

    const totalStudyMin = sessions.reduce((sum: number, s: any) => sum + (s.durationMin || 0), 0);
  ...
  async collectWeeklyData(studentId: number) {
    ...
    for (let d = new Date(weekAgo); d <= today; d.setDate(d.getDate() + 1)) {
      const data = await this.collectDailyScoreData(studentId, d.toISOString().split('T')[0]);
      dailyData.push(data);
    }
