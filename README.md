...
| 서비스 | 경로 | 설명 |
|--------|------|------|
| **GB-Back-Nest** | `E:\Dev\github\GB-Back-Nest` | 인증, 결제, 회원 관리 (공통) |
| **GB-Front** | `E:\Dev\github\GB-Front` | 메인 프론트엔드 (참고용) |
...
# gb-planner 백엔드 (플래너 전용)
VITE_API_URL_PLANNER=http://localhost:4000

# GB-Back-Nest (인증, 결제, 회원 관리)
VITE_API_URL_MAIN=http://localhost:4001
...
```bash
# 프론트엔드만 실행 (http://localhost:3001)
yarn dev

# 백엔드만 실행 (http://localhost:4000)
yarn dev:backend
...
> ⚠️ **중요**: 인증/결제 기능을 사용하려면 GB-Back-Nest 서버도 실행해야 합니다.
> ```bash
> cd E:\Dev\github\GB-Back-Nest && yarn start:dev
> ```
...
| 패키지                     | 설명                           | 포트 |
| -------------------------- | ------------------------------ | ---- |
| `@gb-planner/frontend`     | React + Vite + TanStack Router | 3001 |
| `@gb-planner/backend`      | NestJS + Prisma (플래너 전용)  | 4000 |
...
| 서비스 | 포트 | 설명 |
| ------ | ---- | ---- |
| GB-Back-Nest | 4001 | 인증, 결제, 회원 관리 API |
